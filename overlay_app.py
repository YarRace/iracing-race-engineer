...
Открывает панель управления: галочками включаешь нужные виджеты (педали, топливо,
G-force, дельта, флаги, relative, таблица). Виджеты перетаскиваются мышью и тянутся
за угол; раскладка сохраняется. «Блокировка» — клики проходят в игру.
...
def main():
    ...
    timer.start(33)                                     # ~30 кадров/сек — плавно, без сети
