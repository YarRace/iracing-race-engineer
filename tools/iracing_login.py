...
def main():
    ...
    if not c.login():
        ...
        elif "405" in c.error or "no longer accepts" in c.error:
            print("  Это НЕ твоя ошибка и НЕ ошибка в файле.")
            print("  iRacing убрал вход, которым пользовались программы: теперь")
            print("  он только через форму в браузере. Проверено сегодня:")
            print("    GET  /data/doc  -> 401 (сервер тот и живой)")
            print("    POST /data/doc  -> 401 (POST проходит)")
            print("    POST /auth      -> 405 (вход закрыт)")
            print("  Разбирать их сайт, чтобы обойти это, я не буду.")
            print("  Имя и рейтинг пилота при этом берутся из Garage 61 —")
            print("  строка на главной не пустая.")
