...
Нужен установленный Chrome. Без него — и если браузер не отдал снимок —
скрипт скажет об этом вслух и выйдет с кодом 2: «нечем сделать здесь».
Сборку это не роняет (картинки украшают витрину, программу они не
составляют), но и старую картинку молча не подкладывает.
...
def main():
    ...
    # Код 2, а не 1: НЕ СНЯВШИЙСЯ снимок — это среда, а не ошибка в коде.
    # На раннере безголовый Chrome промахивается по таймауту легко, и из-за
    # одной картинки падала вся цепочка, а с ней сборка релиза и автосборка
    # витрины. Снимки украшают витрину, программу они не составляют.
    # Настоящие поломки (упавший импорт, сломанный виджет) по-прежнему
    # выходят исключением и валят сборку.
