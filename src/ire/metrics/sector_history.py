...
_NOT_DRIVING = 1.8
# Граница той же природы СНИЗУ. Сектор втрое короче типичного — это не
# быстрый круг, а обрезанный замер: сбой таймера секторов или круг, попавший
# в базу дважды с разным ts (дедуп такие не берёт, ts в ключе). По тем же
# 1101 замеру:
#
#     настоящая езда              от ×0.975
#     ПУСТО                       ×0.43 … ×0.975
#     обрезанный замер            ×0.43
#
# Без нижней границы такой замер становился «лучшим» сектором, и заголовок
# объявлял «S2 costs you 17.80s on every single lap» — секунд, которых нет.
_TOO_SHORT = 0.7
...
def clean(laps):
    ...
    Граница есть и СНИЗУ. Сектор втрое короче типичного — не рекорд, а
    обрезанный замер, и без нижней границы он становился «лучшим»: на Монце
    выходило Best 3.78 при типичном 15.92, а на Спа заголовок объявлял
    «S2 стоит тебе 17.80 секунды каждый круг».
    """
    return split_bad(laps)[0]


def split_bad(laps):
    """(чистые, пит и вылеты, обрезанные замеры) по одним и тем же медианам.

    Три корзины, а не две, потому что подпись под таблицей у них разная:
    сказать «left out (pit or off)» про круг со сбитым таймером — неправда,
    и человек пойдёт вспоминать заезд в боксы, которого не было.
    """
    ...
    for l in good:
        v = l["sectors"]
        if any(m > 0 and v[i] > m * _NOT_DRIVING for i, m in enumerate(meds)):
            long_.append(l)
        elif any(m > 0 and v[i] < m * _TOO_SHORT for i, m in enumerate(meds)):
            short.append(l)
        else:
            ok.append(l)
    return ok, long_, short
...
def report(laps):
    ...
    base, pitted, cut = split_bad(good)
    ...
    return {"ok": True, "count": n, "laps": len(good), "clean_laps": len(base),
            ...
            "skipped": len(pitted),
            # Обрезанные замеры считаются ОТДЕЛЬНО: у них другая причина и
            # другая подпись на экране.
            "truncated": len(cut),
