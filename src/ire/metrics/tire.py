...
def camber(inner, outer, much=None):
    """Вердикт по разнице кромок. (вердикт, разница) — число всегда рядом.

    `much` — порог «внутренняя кромка греется заметно сильнее». По
    умолчанию общий CAMBER_MUCH, и он НЕ УНИВЕРСАЛЕН: срабатывает на 2%
    колёс Ferrari 499P, 39% колёс Super Formula Lights и 0% колёс
    Porsche 963 GTP. Разница в машинах, а не в сетапе.

    Поэтому есть `metrics/tyre_baseline`: он считает порог по СОБСТВЕННЫМ
    заездам человека в этой машине и на этой оси и подаёт его сюда. Пока
    заездов мало — остаётся общее число, и карточка об этом пишет прямо.
    """
    much = CAMBER_MUCH if much is None else much
