...
ОТКУДА ПОРОГИ. Они измерены по 32 файлам .ibt из сессий Ярослава (Ferrari
499P и Super Formula Lights, 128 «колесо-сессий»), а не взяты из головы:
...
Отсюда полосы ниже. Это НЕ универсальная истина про все машины — это то,
как выглядят данные, которые у нас есть. Ровно поэтому в ответе всегда
стоит и само число: если полоса ошибается, число всё равно верное.
...
# Ниже этого разница кромок — шум: половина его колёс укладывается в ±1.2°.
CAMBER_NOISE = 2.0
# Выше этого внутренняя кромка греется заметно сильнее внешней: верхние ~10%.
CAMBER_MUCH = 6.0
# Корона: σ по его данным 1.14, 95-й процентиль +2.35.
CROWN_BAND = 2.5
...
def camber(inner, outer):
    """Что говорит разница кромок. (вердикт, разница) — число всегда рядом."""
    if inner is None or outer is None:
        return "unknown", None
    d = round(inner - outer, 1)
    if d < -CAMBER_NOISE:
        # Внешняя кромка горячее — развала не хватает. Это чёткий признак:
        # в его данных отрицательная разница почти не встречается.
        return "not_enough", d
    if d > CAMBER_MUCH:
        return "too_much", d
    if d > CAMBER_NOISE:
